...
async def call_ai(messages: list) -> str:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, json=body, timeout=aiohttp.ClientTimeout(total=30)) as resp:
            ...
            print(f"RÉPONSE MISTRAL : {data}")
            if "choices" in data and len(data["choices"]) > 0:
                return data["choices"][0]["message"]["content"]
            elif "error" in data:
                raise Exception(f"Erreur Mistral : {data['error']}")
            else:
...
async def on_message(message: discord.Message):
    ...
    if content.lower() in ["aide", "help", "?"]:
        ...
        embed.set_footer(text="Mistral AI + DuckDuckGo")
    ...
    async with message.channel.typing():
        ...
        try:
            ...
            elif "insufficient" in error or "balance" in error:
                await message.channel.send("💸 Plus de crédits Mistral, contacte l'admin !")
            else:
